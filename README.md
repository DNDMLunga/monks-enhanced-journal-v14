> # ⚠️ UNOFFICIAL FORK — NOT the real Monk's Enhanced Journal
> **This is a private, personal, unofficial fork. It is not maintained, endorsed, reviewed, or supported by IronMonk (ironmonk88 / ironmonk108), the actual author of Monk's Enhanced Journal.**
>
> All credit for this module's design, features, and the vast majority of the code below belongs entirely to IronMonk. For the real, maintained, official module — support, Discord, Patreon, Ko-fi — go to **https://github.com/ironmonk108/monks-enhanced-journal**.
>
> This fork exists for one narrow reason: getting the module working on Foundry VTT v14 for one personal game, ahead of an official v14 release. It builds on three real, open, **unmerged** community pull requests against the official repo — [#820](https://github.com/ironmonk108/monks-enhanced-journal/pull/820), [#821](https://github.com/ironmonk108/monks-enhanced-journal/pull/821), [#823](https://github.com/ironmonk108/monks-enhanced-journal/pull/823), all by [bularzik](https://github.com/bularzik), not by the maintainer of this fork — plus one additional fix on top: the "Convert to Enhanced Journal" action, when used on a journal entry with zero pages, was creating the new page with an invalid native `type` field, which Foundry v14 rejects outright; this fork creates it as a valid `"text"` type instead, matching the pattern the rest of the codebase already uses correctly.
>
> **Not tested extensively, not supported, use at your own risk. Once the official module ships real v14 support, switch back to the official module and stop using this fork.**
>
> Original license terms are unchanged and apply as-is — see the License section at the bottom of this file and `LICENSE`.
>
> — Everything below this line is the original project's own README, unmodified.
